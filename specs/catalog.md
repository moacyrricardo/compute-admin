...
| 053 | Verb & command contract | ⚪ concern | **identity/measurement half RESOLVED → 055 (app-model epic 055–060); verb & command contract residue stays open here (audited by 060)** · **concern** (17 decisions taken 2026-08-12; 5 questions open) — carries forward **040's "what is an app"** half (040's OQ1, read-time shaping, explicitly **stays with 040**). Today an app is a **loose string** and nothing more: 026's correlation-only `app-name` + `appOps`/`opsForApp` group-by, beside a **taxonomy** (032 enums + 033 compose grouping + 038 dedication) classified server-side then re-joined client-side each poll (040's "derived twice"). Costs already biting: 050 infers the **verb from the action name** (red-team found the collision); **hand-launched/cron non-systemd native** work is undetectable (026 and 033 both find portless work — the residue is narrower than "portless"); classification emits *enums* when a probe needs *which command to propose*; MCP can only enumerate action ids. **Decided:** compose project = **one app**, flat namespace, commands may target a **part**; classification survives as a **declared label stamped once**; **closed verb vocabulary** (start·stop·restart·deploy·status·logs·metrics, MCP speaks these — F1 verb-level tools); **declared apps first-class** but narrowed to hand-launched/cron non-systemd work, **not monitored in v1**, **never auto-retire**, **machine-scoped**, and **declaring is a gated act** (proposal → UI approval, 052's invariant); **declarations split by risk** — identity + **verb** as reserved params **inside the hash**, `role`/`dedication` as un-audited **side-data** outside it (033/036 shape) — with a **new invariant** (*a declaration param is never referenced by an argv token*), since 026's own actions bind `{app-name}` into `systemctl`/`journalctl` argv, one under `sudo -n`; **C2 coexist** with 032/033/038 for v1 (end-state written down); **multi-app commands stay first-class**; **path-derived names allowed** per **028**'s `name = host` precedent (basename only, accepted at approval, raw path never on MCP); **D2** one probe constant with a detector table; 022's **normalisation + alias map** moves here. **Open:** app-name scope/collision + alias rules · declared-vs-discovered reconciliation · **where a declared app persists** (022 left entity promotion as "later work") · a **vocabulary amendment criterion** · whether **"destructive" is a declaration orthogonal to verb** (050 keys its confirm drawer to verb names, so a `migrate.sh` chip gets the *least* friction). **Impact:** closes **none** of 049/050's 15 red-team findings (0 dissolved · 2 answered · 12 untouched · **1 worsened** — the symlink-vs-resolved path); supersedes 050's verb-by-name and makes affixed script names tractable, but a **uniqueness construction still has to be written**. **Sequencing:** 049 proceeds independently; 050 drafts against the axis-A answer. Evidence in `053-assets/` |
| 054 | Lightweight app-metadata model | ⚪ concern | **concern — RESOLVED → 055–060 (app-model epic); 5/5 forks decided 2026-08-27 (all recommended).** **Supersedes the framing of 049 + 050** and **partially supersedes 053** (identity half; confirmed D4 in scope). — a clean-slate step back to a purely-mechanical model of *what an app is* and *how we size it*, deliberately independent of built/spec'd code. Unit = a triple **{app-script, script-folder, app/context-folder}**; one pipeline **Discovery → Mapping → Probing** with a Docker/common-service layer feeding all three. Discovery = ports (`ss`/`/proc` → PID → `exe`/`cwd`/`cmdline`) ∪ docker (`inspect`), cgroup-first routing so a container's overlayfs path never reads as an app-folder; blind spots = DNAT/`docker-proxy` ports + non-listening apps. Mapping = a fixed **wrapper-dir rule** (`scripts`/`bin`/`frontend`/… collapse to the parent) grouping siblings under a context. Probing = disk (`du` app-folder − mounts + volumes; double-count rule), RAM (**PSS** summed, never RSS-in-a-sum), CPU (Δ`utime+stime` rate, one-exec sample), DB (logical+physical, docker-only). Docker layer = **inspect the live engine, don't parse source** (zero new deps). Headline: model holds, **~80 % of the map is unprivileged**, every privileged probe has a labelled fallback. **Open forks (5):** symlink identity key (owns 053's *worsened* symlink-vs-resolved token) · wrapper hop depth · privilege posture · non-listening apps · DB sizing scope. 053's **verb** contract stays separate; they meet where 054's chosen identity anchors 053's declared-app params. Carries a 3-identity mock + feasibility report + decision surface in `054-assets/` |
| 055 | App-model foundations & mapping | ⚪ todo | **[BOL-884]** · **app-model epic (graduates 054), the contract 056–060 consume.** The `{app-script, script-folder, context}` record + resolution seam in a new pure `ContextMapper`: wrapper-dir rule (054 D2 — single hop + marker-gated `.git`/`compose.yaml`/`package.json` second hop, 2-hop cap, boundary clamp forbids `/opt /srv /home /usr /var /`, monorepos group at git root) + symlink key (054 D1 — dedup on physical `realpath`, display logical, promote `*/releases/*`\|`*/versions/*` to logical) + sibling-script enumeration. New fields on `AppPortItem` as un-audited `app_port_list` side-data (no migration, no re-approval, re-discovery-refreshed); cgroup-checked overlayfs guard, only PROCESS/SYSTEMD feed the mapper. **Riders:** spec-015 pin LITERAL + `ActionSnapshot.hash` use the **logical/promoted** path (redeploy-stable, no spurious re-approval), physical drives only `approvedScriptHash`. **S9:** raw path never on MCP (basename-only at approval) + a new arch test forbidding any MCP tool emitting an absolute path (028's deferred hardening; closes the `ListActionsTool` argToken leak). Gate/GateArchTest untouched |
| 056 | App discovery | ⚪ todo | **[BOL-885]** · **app-model epic — the Discovery stage producing the records 055 maps.** Listening ports (`ss`//`proc/net/tcp`+fd-inode fallback → PID → exe/cwd/cmdline) ∪ docker (`inspect`) ∪ **non-listening (054 D4 in scope)** — `systemctl --state=running` MainPID + cron enumeration + `ps -eo args` interpreter scan, emitting the same `AppPortItem` with an empty/sentinel port (interpreter script = `argv[1+]`). **cgroup-first routing** (docker/kubepods/containerd PIDs → Docker branch, never mapped as host folders; only PROCESS/SYSTEMD feed `ContextMapper`); **DNAT/docker-proxy** ports resolve to the container, never a proxy `/proc` path; common-service **fingerprint→catalog→verify** (nginx/pg/mysql/mariadb; `Config.Env`+`Mounts[]` override defaults; dockerized DB shares its compose-project context, standalone = own). No new `DiscovererFamily`; `discover_recipes` only (PENDING_APPROVAL, read-only); all probes S4-safe constant `sh -c`, no-sudo. Gate/S9 untouched; 055's no-raw-path arch test covers 056's fields |
| 057 | App footprint probing | ⚪ todo | **[BOL-886]** · **app-model epic — 054's Probing stage (D3/D5) as `RecipeType.MONITOR` actions; no migration/schema/entity/MCP delta, gate untouched.** Four axes, each a different kind: disk `du -sbx` on the context folder with the **double-count rule** (exclude under-mounts, add each volume once), root/data-root FS only; RAM **Σ PSS** from `smaps_rollup` (never RSS-in-a-sum; RSS = ≤upper-bound fallback); CPU a **rate** (Δ(utime+stime), two samples one SSH exec ÷ measured Δt ÷ CLK_TCK); DB docker-only logical (`docker exec` query) + physical (volume `du`). **Cadence tiers** procfs 30–60s / docker 1–5m / du hourly+, fast never blocks slow. **Privilege (D3):** no-sudo degrade-and-label (`confidence=low`, `permission-denied`, cgroup-not-per-PID) + an on-demand **sudo re-probe** action (gated normally). **041 integration:** `computeOther` untouched — native numerators land on `c.ram/c.cpu/c.disk` over the same `ramMb`/`cores`/`diskBytes` denominators so OTHER auto-subtracts (fills the native disk axis 049 never built); honest-absence = null never 0. Consumes 055+056; feeds 059; standalone-DB → 058 |
| 058 | Standalone database sizing | 🟢 done | **[BOL-887]** · branch `moacyrricardo/bol-887-cpt-058-standalone-database-sizing` (PR #87, stacked on integration-057-061-062) · **app-model epic — the deferred 054 D5 piece (non-docker pg/mysql/mariadb).** Same logical SQL as 057's dockerized path, host-side transport only: postgres via **peer auth** (`sudo -u postgres psql`), mysql/mariadb via **config-file creds** (`~/.my.cnf`, else `sudo mysql --defaults-extra-file=/etc/mysql/debian.cnf`) — app never handles a DB password. Two numbers: logical (param-free engine aggregate) + physical (`du -sbx` on the engine-reported datadir, verified via `SHOW data_directory`/`@@datadir` against 057's catalog, under `timeout/nice/ionice`). Extends `DatabaseDiscoverer`'s **standalone branch** (056 owns dockerized-vs-standalone routing); read-only `sudo=true` MONITOR checks (PENDING_APPROVAL) + NATIVE DATABASE consumer on side-data. Physical→disk axis **only if datadir on root/data-root FS** (else over-subtracts OTHER). Gate/S9 untouched; datadir + `debian.cnf` are S9-secret LITERALs sanitised by 055's arch test. Lower priority; builds after 057 |
| 059 | App-model UI (new surfaces) | ⚪ todo | **[BOL-888]** · **app-model epic — presentation layer in the existing spec-012 vanilla-JS system (NOT a visual reskin — a full identity rewrite is a separate concern).** Three surfaces rendering 055–057 data, no server/migration/MCP/gate change: (1) **discovery grouped by context** (extends `discoverySection` — context cards with 055's `contextDisplay`, sibling-script list, disk/RAM/CPU badges, per-action **Approve→add** via the *existing* approval path, family `.filter-chips` retained); (2) **footprint dashboard** (extends spec-034/041 segmented tri-axis bars + labelled legend); (3) **add-machine**. Reuses `app.js`/`app.css` idioms + spec-043/044 responsive/drawer. Design ref `054-assets/mockup-compute-admin.html` (current identity). Restart-dev-server-for-any-FE-change caveat noted |
| 060 | MCP surface audit | ⚪ todo | **[BOL-889]** · **app-model epic — the capstone, authored & built last.** End-to-end audit/reconciliation of the whole MCP surface once 055–057 land: an **S9 path-leak sweep** across all `mcp/*Tool.java`, gate invariants (`GateArchTest`, "no approve tool"), and the **seam where 054 identity meets 053 verbs** (MCP speaks verbs — 053 F1). An audit **plan** + reconciliation spec (defines checks + any renames/removals), not a pile of new tools |
| 061 | Docker-branch discovery enrichment | ⚪ todo | **[BOL-890]** · **app-model follow-up — graduates 056 deferral item 1 (docker half of Decisions 4 & 5); blocked-by 056.** Enriches the untouched Docker branch via one batched `docker inspect` on the live engine (zero new deps, never parses compose source): reads `Mounts[]` + `NetworkSettings.Ports`/`HostConfig.PortBindings` (**DNAT published-port truth** — resolves the docker-proxy blind spot 056's native branch skips) + whitelisted `Config.Env` + `Config.Image`. **Image-tag fingerprinting** mirrors 056's native `ServiceCatalog.fingerprint→catalog→verify` (`PGDATA`/`MYSQL_DATADIR` from env overrides default, translated through `Mounts[]` to host bytes; image+port ⇒ `confidence=high`). **Dockerized-DB context membership** via synthetic non-path keys `compose:<project>`/`container:<name>` (never collide with native `/`-path keys) so a DB container shares its project context; host data location rides `scriptFolder` for 057's sizing. Emits 056's `AppPortItem` (one per published host port, sentinel 0 portless, path-free `sourceNote`) on a combined `{dockerConsumers,appPortList}` object in the same CLOB — **no migration** (V15). No `DiscovererFamily` change (inspect = more reads on the already-gated `DOCKER` socket). Gate/S9/S4 untouched; feeds 057 + 059 |
| 062 | Native /proc/net/tcp port fallback + fd-inode join | ⚪ todo | **[BOL-891]** · **app-model follow-up — graduates 056 deferral item 2 (+ minor item 3); blocked-by 056.** The native robustness path of the listening sweep. **`/proc/net/tcp{,6}` fallback + fd-inode PID join** (one constant no-input `sh -c`: awk over `st=0A` sockets → hex-port+inode, one `/proc/*/fd` scan → `socket:[inode]`→PID, joined in Java) recovers port→PID attribution when unprivileged `ss users:()` is denied; `parseSs`/`parseNetstat` stop dropping blank-users LISTEN lines. **Honest reach:** foreign sockets recovered as ports only (null PID, `confidence=low`, path-free `sourceNote`) — never dropped; sudo re-probe stays 057/054-D3. **`exe` as fallback context signal** (`readlink /proc/<pid>/exe` when cwd is null/`/`/boundary-root and exe is outside `{/usr,/bin,/sbin,/lib,/snap}`) via the existing `ContextMapper`. Minor: nginx real-root via `nginx -T`; relative interpreter-script resolution against cwd. Same nine-field `AppPortItem`, **no migration** (V15), **no MCP-surface delta** (paths only in S9-secret side-data guarded by `McpPathLeakArchTest`); all probes S4-safe no-sudo |
