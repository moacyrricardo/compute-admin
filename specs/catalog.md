...
| 045 | Architecture cleanups | ⚪ todo | **concern** (options open; researched impact) — four mechanical cleanups, gate untouched: (1) exceptions **extend `WebApplicationException`** to delete the **19** `*ExceptionMapper` classes (~−400 lines); (2) DTOs over generic `Response`/`Request` — **already done** (all 12 RS return typed records, 0 return `Response`; residue = the mappers' `Map.of` bodies → a typed `ErrorResponse`); (3) **`.properties`** instead of the 3 YAML files (65 lines); (4) **uber jar** (already built by spring-boot plugin) + a `v*`-tag **GitHub release** action + README download/usage. ≈ −16 net classes / −350 net lines |
