...
| 043 | Mobile-responsive UI | ⚪ todo | make the web UI usable on phones (≈360–430px) + tablets by **extending** the existing tokens/CSS (no framework; keeps `h()`/textContent-only + both themes). Today only one breakpoint (720px, shell+nav). Adds `--bp-sm 480`, a collapsible **Menu** nav, single-column stacking of `.app-cards`/`.row-between`/monitor controls, a bottom-sheet drawer, ≥44px touch targets, no sideways scroll. Presentation only; gate/data untouched |
