...
| 041 | Host system/other usage segment | ⚪ todo | show **real** RAM/CPU/disk on app-less machines: poll host vitals' **used** (currently only totals are kept, used is discarded), render an **OTHER/system** segment = `host_used − Σ attributed` so a bare box no longer looks idle, keep `total − used` as the hatched free tail. Client-side (spec-040 leaning); needs `monitor machine` vitals. Concrete driver for **040** |
