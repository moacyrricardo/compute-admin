...
| 053 | App model & verb contract | ⚪ concern | **concern** (four axes decided, rest open) — **absorbs 040's "what is an app"** half (its OQ1 *where is the BFF line* + OQ6 *does this unwind 032/033/038*). Today an app is three partial answers that never meet: a **string** (026's correlation-only `app-name` param + `appOps`/`opsForApp` group-by), a **taxonomy** (032 enums + 033 compose grouping + 038 dedication, classified server-side then re-joined client-side each poll — 040's "derived twice"), and **whatever answers on a port** (025→049→050 chain). Consequences already biting: 050 infers the **verb from the action name** (red-team found the name-collision crack); **portless native workloads** (cron/worker/bare bash) are structurally invisible; classification's output is *enums* when what a probe needs is *which command to propose*; MCP can only enumerate action ids, never ask what an app answers to nor declare one. **Decided 2026-08-12:** (1) compose project = **one app**, flat namespace, commands may declare a **part** (ui/worker/db); (2) classification survives as a **small declared label** (`role`/`dedication`) stamped at proposal, never recomputed at read; (3) **closed verb vocabulary** (start·stop·restart·deploy·status·logs·metrics get controls + are what MCP speaks; anything else stays a plain run chip); (4) **declared apps are first-class** — may exist with no detected process ("declared — no process seen"), the only route by which portless work becomes visible. Model: *app = a name (022's declined `App` entity stays declined) · contract = the verbs its APPROVED commands declare, partial by design · classification = a proposal-time input whose **output is commands** · reading declares its own unit + attributability*. Open: where declarations live (reserved params vs columns vs convention), whether they enter the **approval hash** (036: `app-name` ALLOWED_SET **is** hashed), convert-vs-coexist for 032/033/038, probe-constant count under S4, retiring a declared-but-unseen app (needs 036), MCP verb-tools + the **S9 collision** (049 derives identity from paths; its own Known Gaps forbid paths on the MCP surface). Impacts **049/050** (supersedes 050's verb-by-name; changes 049's output contract) and **052** (a transfer aimed at an *app* beats one aimed at a raw path) |
...
**Native app topology (specs 049 → 050, on the spec-040 thin-BE leaning):**

```
049 app-folder & footprint  →  050 lifecycle-script detection & controls
```
