...
| 020 | Machine monitoring | ⚪ todo | **concern** — recipe-driven dashboard w/ client-side poll (default one-shot; 30s/1min); `monitor machine` (mem/disk/cpu) + `springboot monitor` (per-port) recipes; read-only-gate question |
