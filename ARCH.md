...
| **S9** | ~~**MCP infra exposure** — machine `host`/`port`/`loginUser` were surfaced over the MCP surface: `list_machines` returned host/port/loginUser and `register_machine` echoed host, pushing IP/hostnames, SSH login users, and ports into the LLM context (and any MCP-layer logging). Owner-scoped, so not a cross-user leak — needless infra/topology exposure.~~ **RESOLVED by spec 028** — MCP identifies machines by `id` + user-provided `name` (+ `tags`/`status`) only via the dedicated `McpMachineView`; `host`/`port`/`loginUser` stay UI-only on `MachineView`. `register_machine` still *accepts* a host as input but no longer echoes it. | — | — |
